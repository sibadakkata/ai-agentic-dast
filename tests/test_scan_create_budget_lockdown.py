...
def scan_client(fresh_db, monkeypatch):
    ...
    monkeypatch.setenv("AUTO_MODE_DEFAULT_BUDGET_USD", "30")
...
class TestScanCreateBudgetLockdown:
    def test_auto_sso_custom_cap(self, scan_client):
        ...
        _session(c, user.id)
        resp = c.post("/api/scan", json={"target_url": "https://example.com", "model_policy": "auto", "budget_cap_usd": 5.0})
    ...
    def test_auto_sso_no_cap_default_30(self, scan_client):
        ...
        _session(c, user.id)
        resp = c.post("/api/scan", json={"target_url": "https://example.com", "model_policy": "auto"})
        assert resp.status_code == 200
        assert app.SCANS[resp.json()["scan_id"]]["budget_cap_usd"] == 30.0

    def test_auto_basic_auth_cap_ignored(self, scan_client):
        c, app = scan_client
        resp = c.post("/api/scan", json={"target_url": "https://example.com", "model_policy": "auto", "budget_cap_usd": 5.0}, auth=c.auth)
        assert resp.status_code == 200
        data = resp.json()
        assert app.SCANS[data["scan_id"]]["budget_cap_usd"] == 30.0
        assert data.get("budget_override_ignored") is True

    def test_manual_basic_auth_cap_passthrough(self, scan_client):
        c, app = scan_client
        resp = c.post("/api/scan", json={"target_url": "https://example.com", "model_policy": "manual", "budget_cap_usd": 5.0}, auth=c.auth)
        assert resp.status_code == 200
        assert app.SCANS[resp.json()["scan_id"]]["budget_cap_usd"] == 5.0
