...
| **Exploitation Tiers** | Every finding is assigned `validated` (exploitation proven: runtime confirmed, payload reflected, SQL error returned, Garak/LLM-Agent probe failure) or `informational` (detected but not proven: pattern match, missing header, config check). Garak and LLM-Agent findings are always `validated` — the model actually produced the harmful output. Follows "proof over probability" methodology | `_assign_exploitation_tier()`, `_assign_exploitation_tier_garak()` in `triage_engine.py` |
