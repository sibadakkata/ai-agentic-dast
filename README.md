...
| **LLM Application Security** | Auto-detects chatbot/AI-powered features via DOM heuristics and network traffic analysis. Runs 37 deterministic probes covering OWASP Top 10 for LLM Applications (LLM01 Prompt Injection, LLM02 Info Disclosure, LLM05 Output Handling, LLM06 Excessive Agency, LLM07 Prompt Leakage, LLM10 Unbounded Consumption). Optionally runs **Garak (NVIDIA) probe battery** with 48 probe families covering jailbreaks (`dan.DanInTheWild`, `grandma.*`), toxicity/slurs (`lmrc.Profanity`, `lmrc.SlurUsage`), prompt injection (`promptinject.*`), encoding attacks, and more. Garak findings are trusted by the triage engine — the exact payload and chatbot response are shown in the finding narrative. Force with `focus_areas: ["LLM"]` | `llm_detect.py`, `llm_baseline.py`, `garak_runner.py`; Garak is optional (`pip install garak`) |
...
| **Exploitation Tiers** | Every finding is assigned `validated` (exploitation proven: runtime confirmed, payload reflected, SQL error returned, Garak probe failure) or `informational` (detected but not proven: pattern match, missing header, config check). Garak LLM findings are always `validated` — the model actually produced the harmful output. Follows "proof over probability" methodology | `_assign_exploitation_tier()`, `_assign_exploitation_tier_garak()` in `triage_engine.py` |
